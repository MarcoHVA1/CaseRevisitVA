...
if page == "Overzicht":
 ...
 # 7) Correlatie – LIJNGRAFIEK ipv scatter (geen statsmodels nodig)
st.subheader("📈 Correlatie (lijngrafiek) tussen twee variabelen per station")

# Kies variabelen
label_map_full = {"TG_C": "Temperatuur (°C)", "RH_mm": "Neerslag (mm)", "SQ_h": "Zonuren (h)"}
colX, colY = st.columns(2)
x_var = colX.selectbox("Variabele A", ["TG_C", "RH_mm", "SQ_h"], index=0,
                       format_func=lambda k: label_map_full[k])
y_var = colY.selectbox("Variabele B", ["TG_C", "RH_mm", "SQ_h"], index=2,
                       format_func=lambda k: label_map_full[k])

# Aggregatie naar JAAR per station (zodat lijnen tijdreeks zijn)
df_all["year"] = df_all["date"].dt.year
agg_year = (
    df_all.groupby(["station_key", "station", "year"], as_index=False)
    .agg({x_var: "mean", y_var: "mean"})   # tijdreeks vergelijkbaar houden
    .dropna(subset=[x_var, y_var])
)

if agg_year.empty:
    st.info("Geen gegevens beschikbaar om de lijngrafiek te maken met de huidige filters.")
else:
    # Normaliseren per station (min–max 0..1) zodat beide variabelen op één y-as passen
    def minmax_norm(s):
        s = pd.to_numeric(s, errors="coerce")
        lo, hi = s.min(), s.max()
        if pd.isna(lo) or pd.isna(hi) or hi == lo:
            return pd.Series([0.5] * len(s), index=s.index)  # vlakke lijn als er geen variatie is
        return (s - lo) / (hi - lo)

    norm = []
    r_rows = []  # voor r per station
    for sk, g in agg_year.groupby("station_key"):
        g = g.sort_values("year").copy()
        g["A_norm"] = minmax_norm(g[x_var])
        g["B_norm"] = minmax_norm(g[y_var])

        # Pearson r over de jaren (op de rauwe waarden, niet genormaliseerd)
        r = g[x_var].corr(g[y_var])
        r_rows.append({"station": g["station"].iloc[0], "r": r})

        # op lange forma
        gA = g[["station", "year", "A_norm"]].rename(columns={"A_norm": "value"})
        gA["reeks"] = f"{label_map_full[x_var]}"
        gB = g[["station", "year", "B_norm"]].rename(columns={"B_norm": "value"})
        gB["reeks"] = f"{label_map_full[y_var]}"
        norm.append(pd.concat([gA, gB], ignore_index=True))

    norm_df = pd.concat(norm, ignore_index=True)
    r_df = pd.DataFrame(r_rows).dropna()

    # Lijngrafiek: kleur = station, stippel = variabele (A/B)
    fig_corr = px.line(
        norm_df,
        x="year",
        y="value",
        ...
        line_dash="reeks",
        markers=True,
        labels={"year": "Jaar", "value": "Genormaliseerde waarde (0–1)", "reeks": "Variabele"},
        title=f"Correlatie-lijngrafiek: {label_map_full[x_var]} (A) vs {label_map_full[y_var]} (B)"
    )
    fig_corr.update_layout(legend_title_text="Station / Variabele")
    st.plotly_chart(fig_corr, use_container_width=True)

    # Kleine tabel met r per station (optioneel)
    if not r_df.empty:
        r_df = r_df.sort_values("r", ascending=False)
        st.caption("**Pearson r** per station (berekend op jaarwaarden):")
        st.dataframe(r_df.style.format({"r": "{:.2f}"}), use_container_width=True)

st.caption("Beide variabelen zijn per station min–max genormaliseerd (0–1) voor visuele vergelijking op één as. "
           "De tabel toont de Pearson-correlatie r op de ongenormaliseerde jaarwaarden.")
